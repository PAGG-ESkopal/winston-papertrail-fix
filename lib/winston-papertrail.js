...
Papertrail.prototype.sendMessage = function (hostname, program, level, message) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        // Handle all the colorize options...

        var colorLevel = self.colorize === 'level' || self.colorize === true
                ? winston.config.colorize(level) : level;
        var colorLine = self.colorize === 'message'
                ? winston.config.colorize(level, gap + lines[i]) : gap + lines[i];
        var colorText = self.logFormat(colorLevel, colorLine);
        if (self.colorize === 'all') {
            colorText = winston.config.colorize(level, colorText);
        }
